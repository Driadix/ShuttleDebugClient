...
const LogLine = ({ log }) => {
  ...
  return <p className={color}>{log}</p>;
};

const LiveLogViewer = ({ logs, onSaveLog, onClearLogs }) => { // Expect onClearLogs from parent
    ...
    <div className="flex flex-col w-full lg:w-[70%] bg-panel-light dark:bg-panel-dark rounded-lg border border-border-light dark:border-border-dark overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto font-mono text-xs leading-relaxed">
        <Virtuoso
          ...
          className="h-full w-full" // Ensure Virtuoso fills the container
